...
### 5. Utiliser la File d'Attente ThreadPool et la Gestion du Contexte Flask

#### Étape 1: Envoi Synchrone via ThreadPoolQueue

La `ThreadPoolQueue` est utilisée pour exécuter des tâches de manière synchrone dans des threads séparés. Cela est particulièrement utile pour éviter les problèmes de boucle d'événements imbriqués dans Flask.

```python
# core/queue.py
from core.queue import thread_pool_queue

# Envoi synchrone d'une notification via la ThreadPoolQueue
task_id = thread_pool_queue.enqueue(
    async_queue.send_notification_sync,
    app_instance=app, # Instance de l'application Flask nécessaire pour le contexte
    user_id=123,
    title="Notification Importante",
    body="Ceci est un message important",
    emergency_type="sécurité"
...
print(f"Tâche ajoutée à la file ThreadPool: {task_id}")
```

#### Étape 2: Gestion du Contexte Flask dans les Threads

Lors de l'exécution de tâches dans des threads, il est crucial de gérer le contexte de l'application Flask pour accéder à la base de données et à d'autres ressources.
...
#### Étape 3: Intégration avec l'Application Flask

L'application Flask est configurée pour utiliser la `ThreadPoolQueue` pour envoyer des notifications de manière asynchrone via la route `/send-notification`.
...
def send_notification():
    ...
    try:
        task_id = thread_pool_queue.enqueue(
            async_queue.send_notification_sync,
            app_instance=app, # Passer l'instance de l'application Flask
            user_id=user.id,
            title=title,
            body=body,
...
# Tests FastAPI spécifiques
pytest tests/test_fastapi_routes.py
```
...
### 2. Production avec Gunicorn et Uvicorn

```bash
# Installer Gunicorn et Uvicorn
pip install gunicorn uvicorn

# Lancer l'application Flask
gunicorn --bind 0.0.0:5000 --workers 4 app:app

# Lancer l'API FastAPI
uvicorn api.main:app --host 0.0.0.0 --port 800 --workers 4
```
...
```ini
# Flask
[program:flashnotify-flask]
directory=/path/to/app
command=/path/to/venv/bin/gunicorn -w 4 -b 127.0.0.1:5000 app:app
autostart=true
autorestart=true
stderr_logfile=/var/log/flashnotify-flask/err.log
stdout_logfile=/var/log/flashnotify-flask/out.log

# FastAPI
[program:flashnotify-fastapi]
directory=/path/to/app
command=/path/to/venv/bin/uvicorn api.main:app --host 127.0.1 --port 8000 --workers 4
autostart=true
autorestart=true
stderr_logfile=/var/log/flashnotify-fastapi/err.log
stdout_logfile=/var/log/flashnotify-fastapi/out.log

# Celery Worker
[program:flashnotify-celery]
directory=/path/to/app
command=/path/to/venv/bin/celery -A core.queue worker --loglevel=info
autostart=true
autorestart=true
stderr_logfile=/var/log/flashnotify-celery/err.log
stdout_logfile=/var/log/flashnotify-celery/out.log
```
...
CMD ["sh", "-c", "python app.py & uvicorn api.main:app --host 0.0.0.0 --port 800 --reload"]
```
...
- Utiliser les files d'attente asynchrones
- Optimiser les requêtes SQL
- Utiliser des workers multiples

### Problème: Authentification Échoue

**Symptôme:** `401 Unauthorized` pour les routes protégées
...
headers = {
    ...
    # ou
    'Authorization': 'API-Key votre_api_key'
    # ou
    'Authorization': 'Session votre_session_token'
}
```

### Problème: Files d'Attente Ne Fonctionnent Pas

**Symptôme:** Les tâches ne sont pas exécutées

**Solution:**
```python
# Vérifier que les files sont démarrées
from core.queue import async_queue, thread_pool_queue
...
- 📧 Email: dev@flashnotify.com
- 🐛 Issues: https://github.com/flashnotify/issues
- 📚 Documentation: https://docs.flashnotify.com
