...
Pour lancer l'application FlashNotify, plusieurs options s'offrent à vous :

### Avec Docker Compose (recommandé)

```bash
# Lancer l'application complète (base de données, Redis, application, Nginx)
docker-compose up -d

# Accéder à l'application
# - Interface web Flask : http://localhost:80 (ou http://localhost:8000)
# - API FastAPI : http://localhost:80 (ou http://localhost:8000) sur le chemin /api/v2/
...
# Lancer l'API FastAPI (port 8000 par défaut)
python run_fastapi.py

# Dans un autre terminal, lancer l'application Flask (port 5000 par défaut)
python app.py

## Remarque importante sur Docker

La commande suivante est **incorrecte** et générera une erreur :

```bash
# NE PAS FAIRE - Cette commande ne fonctionnera pas
docker run -d -p 8000:8000 --flashnotify
```

Le flag `--flashnotify` n'existe pas dans Docker. Pour lancer l'application FlashNotify correctement avec Docker, utilisez :

```bash
# Lancer avec Docker Compose (méthode recommandée)
docker-compose up -d

# Ou construire et lancer l'image Docker directement
