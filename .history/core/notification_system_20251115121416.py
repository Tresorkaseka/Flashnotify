    ...
    def archive_notification(self, notification_data):
        ...
        try:
            ...
            with current_app.app_context():
                logging.info(f"Tentative d'archivage de la notification: {notification_data['title']}")
                ...
                logging.info(f"Notification archivée avec succès, ID: {notification.id}")
                
            return notification_data
            
        except Exception as e:
            logging.error(f"Erreur lors de l'archivage de la notification: {e}", exc_info=True)
            try:
                db.session.rollback()
            except Exception as rb_e:
                logging.error(f"Erreur lors du rollback de la session: {rb_e}")
            return notification_data
