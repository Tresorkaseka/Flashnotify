...
def login_required(f):
    ...
    def decorated_function(*args, **kwargs):
        ...
        # Stocke les données de session pour utilisation dans la vue
    ...
    with app.app_context():
        ...
        if User.query.count() == 0:
            ...
            admin_user = User(
                ...
                phone=None,
                password='',  # Will be set after object creation
                role='admin',
                prefers_email=True
            )
            # Set the hashed password after object creation
            admin_user.password = auth_manager.hash_password('admin123')
            db.session.add(admin_user)
            
            # Créer des utilisateurs de test
            users = [
                User(name='Alice Martin', email='alice.martin@universite.edu', phone='+33612345678',
                     password='', role='user', prefers_email=True),
                User(name='Bob Dupont', email='bob.dupont@universite.edu', phone='+33623456789',
                     password='', role='user', prefers_email=False),
                User(name='Charlie Bernard', email='charlie.bernard@universite.edu',
                     password='', role='user', prefers_email=True),
            ]
            
            # Set hashed passwords after object creation
            for user in users:
                user.password = auth_manager.hash_password('user123')
                db.session.add(user)
