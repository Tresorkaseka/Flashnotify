...
def _run_async_loop(loop):
    ...
    try:
        ...
        logging.error(f"Erreur dans la boucle asyncio: {e}")
    finally:
...
def init_queues():
    ...
    # Démarrage de la file d'attente asynchrone dans un thread dédié
    loop = asyncio.new_event_loop()
    t = threading.Thread(target=_run_async_loop, args=(loop,), daemon=True)
    t.start()
    
    logging.info("Files d'attente initialisées (Async & ThreadPool)")
