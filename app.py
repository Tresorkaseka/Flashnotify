...
@login_required
def admin():
    """Page d'administration des utilisateurs, notifications et file d'attente"""
    users = User.query.all()
    
    # Récupérer les notifications récentes pour afficher dans le dashboard admin
    recent_notifications = Notification.query.order_by(Notification.created_at.desc()).limit(10).all()
    logging.info(f"Admin dashboard: {len(recent_notifications)} recent notifications fetched.")
    ...
    return render_template('admin.html',
                           ...
                           queue_status_data=queue_status_data)
...
def queue_status():
    ...
    # Récupérer les notifications récentes pour afficher dans le dashboard
    recent_notifications = Notification.query.order_by(Notification.created_at.desc()).limit(10).all()
...
def clear_queue():
    ...
    try:
        ...
        # Redémarrage de la file d'attente
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        try:
